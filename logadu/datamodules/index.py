...
class IndexDataModule(pl.LightningDataModule):
    def __init__(self, dataset_file: str, split_method: int = 1, window_size: int = 10, batch_size: int = 128, label_type: str = 'next'):
    ...
    def _generate_sequences_dedup(self, df):
        ...
        # 3. remove duplicates from sequences and next events that are the same, keeping the first occurrenc
        # data = data.drop_duplicates(subset=['sequence', 'next'], keep='first') this approach gives an error because the sequence is a list, not a hashable type, let's create a key to use it for deduplication and then drop duplicates, seq_next_key
        ...
        data = data.drop(columns=['seq_next_key'])
