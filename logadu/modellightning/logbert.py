...
class LogBERTLightning(pl.LightningModule):
    def __init__(self, vocab_size, embedding_dim, hidden_size, num_layers, num_attention_heads, 
        ...
        self.model = LogBERT(
            vocab_size=vocab_size, embedding_dim=embedding_dim, hidden_size=hidden_size,
        ...
        # Register 'center' as a buffer. It's part of the model's state but not a trainable parameter.
        self.register_buffer("center", torch.zeros(self.hparams.embedding_dim))
        self.nu = 1e-3 # A small constant for the moving average update
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 3. Combine the losses as described in the paper
        ...
        # 4. Update the center `c` using a stable moving average
        with torch.no_grad():
